...
public class FactoryManagerTest {
    ...
    /**
     * Tests creating a user with the factory and retrieving it with the manager.
     */
    /*
    private static void testCreateAndRetrieveUser(UserFactory userFactory, UserManager userManager) {
        System.out.println("\n=== Testing User Creation and Retrieval ===");

        // Create test user data
        String username = "testuser" + System.currentTimeMillis(); // Ensure unique username
        String email = "test" + System.currentTimeMillis() + "@example.com"; // Ensure unique email
        String password = "password123";
        String firstName = "Test";
        String lastName = "User";

        // Create a free user
        User user = userFactory.createUserWithPersonalInfo(
                username,
                email,
                password,
                firstName,
                ...
                new Date(),
                false, // not premium
                0 // subscription days (not used for free users)
        );
        System.out.println("Created user: " + user.getUsername() + " (ID: " + user.getUserID() + ")");

        // Retrieve user by ID
        User retrievedUserById = userManager.getUserById(user.getUserID());

        // Retrieve user by username
        User retrievedUserByUsername = userManager.getUserByUsername(username);

        // Validate retrieval
        boolean idCheckPassed = retrievedUserById != null && retrievedUserById.getUsername().equals(username);
        boolean usernameCheckPassed = retrievedUserByUsername != null && retrievedUserByUsername.getUserID() == user.getUserID();

        System.out.println("Retrieval by ID: " + (idCheckPassed ? "PASSED" : "FAILED"));
        System.out.println("Retrieval by username: " + (usernameCheckPassed ? "PASSED" : "FAILED"));
    }
*/
    /**
     * Tests creating and managing user follow relationships.
     */
    /*
    private static void testUserFollowing(UserFactory userFactory, UserManager userManager) {
        System.out.println("\n=== Testing User Following Relationships ===");

        // Create two users
        User user1 = userFactory.createFreeUser(
                "follower" + System.currentTimeMillis(),
                "follower" + System.currentTimeMillis() + "@example.com",
                "password"
        );

        User user2 = userFactory.createFreeUser(
                "followed" + System.currentTimeMillis(),
                "followed" + System.currentTimeMillis() + "@example.com",
                "password"
        );
        ...
        boolean success = userFactory.createFollowRelationship(user1.getUserID(), user2.getUserID());
        System.out.println("Created follow relationship: " + (success ? "YES" : "NO"));

        // Refresh users to ensure we have latest data
        userManager.refreshCache();

        // Check if follow relationship exists
        List<User> followed = userManager.getFollowedUsers(user1.getUserID());
        List<User> followers = userManager.getFollowers(user2.getUserID());

        boolean user1FollowsUser2 = followed.stream()
                .anyMatch(u -> u.getUserID() == user2.getUserID());

        boolean user2IsFollowedByUser1 = followers.stream()
                .anyMatch(u -> u.getUserID() == user1.getUserID());

        System.out.println("User1 follows User2: " + (user1FollowsUser2 ? "PASSED" : "FAILED"));
        System.out.println("User2 is followed by User1: " + (user2IsFollowedByUser1 ? "PASSED" : "FAILED"));
    }
*/
    /**
